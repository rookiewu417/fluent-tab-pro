...
function clearEarlyWallpaperBootstrap(): void {
    ...
    document.documentElement.removeAttribute('data-early-wallpaper-blur');
}
...
async function getOptimizedApiWallpaper(remoteUrl: string, source: string): Promise<string> {
    ...
    try {
        ...
        console.warn(`Fetch or save failed for ${source}, using original URL.`, error);
        return remoteUrl;
    }
}
...
async function loadCustomWallpaper() {
    ...
    try {
        let url = (window as any).__earlyWallpaperUrl;
        if (url) {
            delete (window as any).__earlyWallpaperUrl;
